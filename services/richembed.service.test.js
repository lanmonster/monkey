...
/* eslint-disable */
const mpIconUrl =
    'https://is5-ssl.mzstatic.com/image/thumb/Purple118/v4/a4/81/d2/a481d2af-405f-b611-fabb-d7875e97b086/AppIcon-0-1x_U007emarketing-0-0-85-220-0-7.png/246x0w.jpg';
    ...
    describe('createForUser', () => {
        it('should create RichEmbed for user data', () => {
            ...
            const expected = new Discord.RichEmbed()
                .setColor('#7286D5')
                .setAuthor(
                    testdata.fullMpUserData.name,
                    mpIconUrl,
                    testdata.fullMpUserData.url,
                )
                .setThumbnail(testdata.fullMpUserData.avatar)
                .setDescription(fullDescription)
                .addField(
                    'Boulders',
                    testdata.fullMpUserData.styles.Boulders.lead,
                )
                .addField(
                    'Trad Lead',
                    testdata.fullMpUserData.styles.Trad.lead,
                    true,
                )
                .addField(
                    'Trad Follow',
                    testdata.fullMpUserData.styles.Trad.follow,
                    true,
                )
                .addField(
                    'Sport Lead',
                    testdata.fullMpUserData.styles.Sport.lead,
                    true,
                )
                .addField(
                    'Sport Follow',
                    testdata.fullMpUserData.styles.Sport.follow,
                    true,
                )
                .addField(
                    'Aid Lead',
                    testdata.fullMpUserData.styles.Aid.lead,
                    true,
                )
                .addField(
                    'Aid Follow',
                    testdata.fullMpUserData.styles.Aid.follow,
                    true,
                )
                .addField(
                    'Ice Lead',
                    testdata.fullMpUserData.styles.Ice.lead,
                    true,
                )
                .addField(
                    'Ice Follow',
                    testdata.fullMpUserData.styles.Ice.follow,
                    true,
                )
                .addField(
                    'Mixed Lead',
                    testdata.fullMpUserData.styles.Mixed.lead,
                    true,
                )
                .addField(
                    'Mixed Follow',
                    testdata.fullMpUserData.styles.Mixed.follow,
                    true,
                )
                .setFooter(
                    `Joined Mountain Project on ${testdata.fullMpUserData.memberSince}`,
                );
        ...
        it('should not have fields for empty values', () => {
            ...
            const expected = new Discord.RichEmbed()
                .setColor('#7286D5')
                .setAuthor(
                    testdata.mpUserData.name,
                    mpIconUrl,
                    testdata.mpUserData.url,
                )
                ...
                .addField(
                    'Sport Lead',
                    testdata.mpUserData.styles.Sport.lead,
                    true,
                )
                .addField(
                    'Sport Follow',
                    testdata.mpUserData.styles.Sport.follow,
                    true,
                )
                .setFooter(
                    `Joined Mountain Project on ${testdata.mpUserData.memberSince}`,
                );
        ...
        it('should not have sections in the description for empty values', () => {
            ...
            const expected = new Discord.RichEmbed()
                ...
                .setDescription(
                    `Favorite Climbs: ${input.favoriteClimbs}\nBlurb: ${input.personalText}`,
                )
        ...
        it('should handle an empty object being passed', () => {
            let actual;
            try {
                testObject.createForUser({});
            } catch (error) {
                actual = error;
            }
            expect(actual).toEqual(new Error('Unknown user!'));
        });
    });
    describe('createForRoutes', () => {
        ...
        it('should say `Boulder problem` if pitches is empty string and rating starts with V', () => {
            ...
            const expected = new Discord.RichEmbed()
                ...
                .setAuthor(route.name, mpIconUrl, route.url)
                .setDescription(
                    `Boulder problem in ${route.location.join(' > ')}`,
                )
                .setThumbnail(route.imgSqSmall)
                .addField('Style', route.type, true)
                .addField('Grade', route.rating, true)
    ...
    describe('createForHelp', () => {
        ...
        it('should create richEmbed for get id', () => {
            ...
            const expected = new Discord.RichEmbed()
                ...
                .setAuthor('!mp get')
                .setDescription(
                    'Gets the userId or email you previously saved.',
                )
        ...
        it('should return array of all richEmbeds if `all` is passed', () => {
            ...
            expect(actual.length).toEqual(
                Object.keys(COMMAND_NAMES).length - 1,
            );
            actual.forEach((item) => {
